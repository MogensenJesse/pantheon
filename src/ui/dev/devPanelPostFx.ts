...
import { devSettings, type PostFxCohesionDevSettings, type PostFxGradeDevSettings } from '../../core/GameState';
...
interface GradeSpec extends RangeSpec {
  key: keyof Pick<
...
export function initDevPanelPostFx(_panel: HTMLDivElement, postFX: PostFXContext): () => void {
  ...
  return () => {
    ...
    if (lutEnabled && onLutEnabledChange) lutEnabled.removeEventListener('change', onLutEnabledChange);
    if (lutVendorSelect && onLutVendorChange) {
      lutVendorSelect.removeEventListener('change', onLutVendorChange);
    }
    if (lutPickSelect && onLutPickChange) lutPickSelect.removeEventListener('change', onLutPickChange);
