...
import { PROP_ROCK_KEYS, PROP_STRUCTURE_KEYS, PROP_TREE_KEYS } from './propShadowKeys';
...
export function propReflectsInWater(key: string): boolean {
  ...
  return PROP_TREE_KEYS.has(key) || PROP_ROCK_KEYS.has(key) || PROP_STRUCTURE_KEYS.has(key);
}
