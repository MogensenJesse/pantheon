...
export const PROP_STRUCTURE_KEYS = new Set<string>(
  ASSET_MANIFEST.structures.map((structure) => structure.key),
);

/** Whether a map prop asset key should cast sun shadows when instanced. */
export function propCastsShadow(key: string): boolean {
  return (
    PROP_TREE_KEYS.has(key) || PROP_ROCK_KEYS.has(key) || PROP_STRUCTURE_KEYS.has(key)
  );
}
