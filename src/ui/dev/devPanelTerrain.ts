...
function logTerrainDispNyquist(): void {
  ...
  const vertexSpacingM = WORLD.SIZE / meshSegments;

  for (const key of TERRAIN_ATLAS_BIOME_KEYS) {
    const repeat = biomes[key].tileRepeat;
    const tilePeriodM = 1 / repeat;
    const texelsPerVertex = (DETAIL_DISP_TILE / tilePeriodM) * vertexSpacingM;
    console.info(
      `[terrain] ${key} disp Nyquist: ~${texelsPerVertex.toFixed(1)} texels/vertex (target 8–16)`,
    );
  }
}
