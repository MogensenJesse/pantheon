// src/dev/panel/sky/devPanelCloudsSpecs.ts — RangeSpec tables for procedural cloud dev panel
import { VISUAL } from '../../../config/visualTuning';
import type { CloudSettings } from '../../../rendering/clouds/cloudConfig';
import type { RangeSpec } from '../../bindRange';

const C = VISUAL.clouds;

export interface CloudSpec extends RangeSpec {
  key: keyof CloudSettings;
}

/** Baked at generation — changing these rebuilds the instanced field. */
export const CLOUD_LAYOUT_SPECS: CloudSpec[] = [
  {
    id: 'dev-cloud-seed',
    label: 'Seed',
    min: 0,
    max: 99999,
    step: 1,
    defaultValue: C.seed,
    format: (v) => v.toFixed(0),
    key: 'seed',
  },
  {
    id: 'dev-cloud-spread',
    label: 'Spread (m)',
    min: 200,
    max: 3200,
    step: 10,
    defaultValue: C.spread,
    format: (v) => v.toFixed(0),
    key: 'spread',
  },
  {
    id: 'dev-cloud-max-instances',
    label: 'Max instances',
    min: 200,
    max: 4000,
    step: 50,
    defaultValue: C.maxInstances,
    format: (v) => v.toFixed(0),
    key: 'maxInstances',
  },
  {
    id: 'dev-cloud-coverage-noise-cell',
    label: 'Coverage noise cell (m)',
    min: 64,
    max: 512,
    step: 8,
    defaultValue: C.coverageNoise.cellM,
    format: (v) => v.toFixed(0),
    key: 'coverageNoiseCellM',
  },
  {
    id: 'dev-cloud-coverage-noise-softness',
    label: 'Coverage noise softness',
    min: 0.02,
    max: 0.6,
    step: 0.01,
    defaultValue: C.coverageNoise.softness,
    format: (v) => v.toFixed(2),
    key: 'coverageNoiseSoftness',
  },
  {
    id: 'dev-cloud-low-base-y',
    label: 'Low base Y (m)',
    min: 10,
    max: 200,
    step: 1,
    defaultValue: C.layers.low.baseY,
    format: (v) => v.toFixed(0),
    key: 'lowBaseY',
  },
  {
    id: 'dev-cloud-low-jitter',
    label: 'Low altitude jitter (m)',
    min: 0,
    max: 120,
    step: 1,
    defaultValue: C.layers.low.jitter,
    format: (v) => v.toFixed(0),
    key: 'lowAltitudeJitter',
  },
  {
    id: 'dev-cloud-low-count',
    label: 'Low cloud count',
    min: 0,
    max: 80,
    step: 1,
    defaultValue: C.layers.low.cloudCount,
    format: (v) => v.toFixed(0),
    key: 'lowCloudCount',
  },
  {
    id: 'dev-cloud-low-particles-min',
    label: 'Low particles min',
    min: 4,
    max: 40,
    step: 1,
    defaultValue: C.layers.low.particlesMin,
    format: (v) => v.toFixed(0),
    key: 'lowParticlesMin',
  },
  {
    id: 'dev-cloud-low-particles-max',
    label: 'Low particles max',
    min: 4,
    max: 48,
    step: 1,
    defaultValue: C.layers.low.particlesMax,
    format: (v) => v.toFixed(0),
    key: 'lowParticlesMax',
  },
  {
    id: 'dev-cloud-low-size-mul',
    label: 'Low size mul',
    min: 0.25,
    max: 4,
    step: 0.05,
    defaultValue: C.layers.low.sizeMul,
    format: (v) => v.toFixed(2),
    key: 'lowSizeMul',
  },
  {
    id: 'dev-cloud-high-base-y',
    label: 'High base Y (m)',
    min: 100,
    max: 800,
    step: 5,
    defaultValue: C.layers.high.baseY,
    format: (v) => v.toFixed(0),
    key: 'highBaseY',
  },
  {
    id: 'dev-cloud-high-jitter',
    label: 'High altitude jitter (m)',
    min: 0,
    max: 200,
    step: 1,
    defaultValue: C.layers.high.jitter,
    format: (v) => v.toFixed(0),
    key: 'highAltitudeJitter',
  },
  {
    id: 'dev-cloud-high-count',
    label: 'High cloud count',
    min: 0,
    max: 60,
    step: 1,
    defaultValue: C.layers.high.cloudCount,
    format: (v) => v.toFixed(0),
    key: 'highCloudCount',
  },
  {
    id: 'dev-cloud-high-particles-min',
    label: 'High particles min',
    min: 4,
    max: 40,
    step: 1,
    defaultValue: C.layers.high.particlesMin,
    format: (v) => v.toFixed(0),
    key: 'highParticlesMin',
  },
  {
    id: 'dev-cloud-high-particles-max',
    label: 'High particles max',
    min: 4,
    max: 64,
    step: 1,
    defaultValue: C.layers.high.particlesMax,
    format: (v) => v.toFixed(0),
    key: 'highParticlesMax',
  },
  {
    id: 'dev-cloud-high-size-mul',
    label: 'High size mul',
    min: 0.5,
    max: 6,
    step: 0.05,
    defaultValue: C.layers.high.sizeMul,
    format: (v) => v.toFixed(2),
    key: 'highSizeMul',
  },
];

/** Per-frame tunables — no field rebuild. */
export const CLOUD_RUNTIME_SPECS: CloudSpec[] = [
  {
    id: 'dev-cloud-opacity',
    label: 'Opacity',
    min: 0,
    max: 1,
    step: 0.01,
    defaultValue: C.opacity,
    format: (v) => v.toFixed(2),
    key: 'opacity',
  },
  {
    id: 'dev-cloud-edge-fade',
    label: 'Domain edge fade (m)',
    min: 0,
    max: 300,
    step: 5,
    defaultValue: C.edgeFadeM,
    format: (v) => v.toFixed(0),
    key: 'edgeFadeM',
  },
  {
    id: 'dev-cloud-facing-pow',
    label: 'Rim soft pow (higher softer)',
    min: 0.5,
    max: 4,
    step: 0.05,
    defaultValue: C.facingPow,
    format: (v) => v.toFixed(2),
    key: 'facingPow',
  },
  {
    id: 'dev-cloud-edge-soft',
    label: 'Rim soft width',
    min: 0.05,
    max: 1,
    step: 0.01,
    defaultValue: C.edgeSoftness,
    format: (v) => v.toFixed(2),
    key: 'edgeSoftness',
  },
  {
    id: 'dev-cloud-wind-speed',
    label: 'Wind speed',
    min: 0,
    max: 24,
    step: 0.1,
    defaultValue: C.windSpeed,
    format: (v) => v.toFixed(1),
    key: 'windSpeed',
  },
  {
    id: 'dev-cloud-wind-dir',
    label: 'Wind direction (deg)',
    min: 0,
    max: 360,
    step: 1,
    defaultValue: C.windDirectionDeg,
    format: (v) => v.toFixed(0),
    key: 'windDirectionDeg',
  },
];

/** Soft-particle blob + valley-fog-style wisps — live uniforms. */
export const CLOUD_WISP_SPECS: CloudSpec[] = [
  {
    id: 'dev-cloud-radial-soft',
    label: 'Soft blob',
    min: 0,
    max: 1,
    step: 0.01,
    defaultValue: C.radialSoftness,
    format: (v) => v.toFixed(2),
    key: 'radialSoftness',
  },
  {
    id: 'dev-cloud-wisp-strength',
    label: 'Wisp strength',
    min: 0,
    max: 1,
    step: 0.01,
    defaultValue: C.wispStrength,
    format: (v) => v.toFixed(2),
    key: 'wispStrength',
  },
  {
    id: 'dev-cloud-wisp-scale-a',
    label: 'Wisp scale A',
    min: 0.01,
    max: 0.2,
    step: 0.005,
    defaultValue: C.wispScaleA,
    format: (v) => v.toFixed(3),
    key: 'wispScaleA',
  },
  {
    id: 'dev-cloud-wisp-scale-b',
    label: 'Wisp scale B',
    min: 0.02,
    max: 0.6,
    step: 0.005,
    defaultValue: C.wispScaleB,
    format: (v) => v.toFixed(3),
    key: 'wispScaleB',
  },
  {
    id: 'dev-cloud-wisp-speed',
    label: 'Wisp speed',
    min: 0,
    max: 0.6,
    step: 0.01,
    defaultValue: C.wispSpeed,
    format: (v) => v.toFixed(2),
    key: 'wispSpeed',
  },
  {
    id: 'dev-cloud-corner-radius',
    label: 'Corner radius',
    min: 0,
    max: 1,
    step: 0.01,
    defaultValue: C.cornerRadius,
    format: (v) => v.toFixed(2),
    key: 'cornerRadius',
  },
  {
    id: 'dev-cloud-turbulence',
    label: 'Turbulence',
    min: 0,
    max: 1,
    step: 0.01,
    defaultValue: C.turbulence,
    format: (v) => v.toFixed(2),
    key: 'turbulence',
  },
  {
    id: 'dev-cloud-light-flatten',
    label: 'Light flatten',
    min: 0,
    max: 1,
    step: 0.01,
    defaultValue: C.lightFlatten,
    format: (v) => v.toFixed(2),
    key: 'lightFlatten',
  },
];

/** Terrain hug + soft-fade — live uniforms + per-frame lift. */
export const CLOUD_TERRAIN_SPECS: CloudSpec[] = [
  {
    id: 'dev-cloud-terrain-clearance',
    label: 'Terrain clearance (m)',
    min: 0,
    max: 40,
    step: 0.5,
    defaultValue: C.terrainClearanceM,
    format: (v) => v.toFixed(1),
    key: 'terrainClearanceM',
  },
  {
    id: 'dev-cloud-terrain-fade',
    label: 'Terrain fade below (m)',
    min: 0,
    max: 40,
    step: 0.5,
    defaultValue: C.terrainFadeBelowM,
    format: (v) => v.toFixed(1),
    key: 'terrainFadeBelowM',
  },
];

/** Lighting cohesion + valley haze + shadow receive — live uniforms. */
export const CLOUD_LIGHTING_SPECS: CloudSpec[] = [
  {
    id: 'dev-cloud-haze-mix',
    label: 'Night valley haze mix',
    min: 0,
    max: 1,
    step: 0.01,
    defaultValue: C.hazeMix,
    format: (v) => v.toFixed(2),
    key: 'hazeMix',
  },
  {
    id: 'dev-cloud-shadow-floor',
    label: 'Shadow floor',
    min: 0,
    max: 1,
    step: 0.01,
    defaultValue: C.shadowFloor,
    format: (v) => v.toFixed(2),
    key: 'shadowFloor',
  },
  {
    id: 'dev-cloud-shadow-lift',
    label: 'Shadow sample lift (m)',
    min: 0,
    max: 20,
    step: 0.5,
    defaultValue: C.shadowSampleLiftM,
    format: (v) => v.toFixed(1),
    key: 'shadowSampleLiftM',
  },
  {
    id: 'dev-cloud-light-scale-min',
    label: 'Light scale floor',
    min: 0,
    max: 0.4,
    step: 0.01,
    defaultValue: C.lightScaleMin,
    format: (v) => v.toFixed(2),
    key: 'lightScaleMin',
  },
  {
    id: 'dev-cloud-golden-tint',
    label: 'Golden tint strength',
    min: 0,
    max: 1,
    step: 0.01,
    defaultValue: C.goldenTintStrength,
    format: (v) => v.toFixed(2),
    key: 'goldenTintStrength',
  },
  {
    id: 'dev-cloud-sun-catch',
    label: 'Sun catch (lit face)',
    min: 0,
    max: 1,
    step: 0.01,
    defaultValue: C.sunCatchStrength,
    format: (v) => v.toFixed(2),
    key: 'sunCatchStrength',
  },
  {
    id: 'dev-cloud-mass-normal-mix',
    label: 'Mass normal mix',
    min: 0,
    max: 1,
    step: 0.01,
    defaultValue: C.massNormalMix,
    format: (v) => v.toFixed(2),
    key: 'massNormalMix',
  },
  {
    id: 'dev-cloud-base-shade',
    label: 'Base shade',
    min: 0,
    max: 0.6,
    step: 0.01,
    defaultValue: C.baseShade,
    format: (v) => v.toFixed(2),
    key: 'baseShade',
  },
  {
    id: 'dev-cloud-self-shadow',
    label: 'Mass self-shadow',
    min: 0.5,
    max: 4,
    step: 0.05,
    defaultValue: C.selfShadow,
    format: (v) => v.toFixed(2),
    key: 'selfShadow',
  },
  {
    id: 'dev-cloud-silver-strength',
    label: 'Silver lining',
    min: 0,
    max: 1.5,
    step: 0.01,
    defaultValue: C.silverStrength,
    format: (v) => v.toFixed(2),
    key: 'silverStrength',
  },
  {
    id: 'dev-cloud-silver-g',
    label: 'Silver HG g',
    min: 0,
    max: 0.95,
    step: 0.01,
    defaultValue: C.silverG,
    format: (v) => v.toFixed(2),
    key: 'silverG',
  },
];

/** Energy/atmosphere reveal opacity ramp (not day/night — night stays visible). */
export const CLOUD_REVEAL_SPECS: CloudSpec[] = [
  {
    id: 'dev-cloud-reveal-min',
    label: 'Reveal min opacity',
    min: 0,
    max: 1,
    step: 0.01,
    defaultValue: C.revealMinCoverage,
    format: (v) => v.toFixed(2),
    key: 'revealMinCoverage',
  },
  {
    id: 'dev-cloud-reveal-max',
    label: 'Reveal max opacity',
    min: 0,
    max: 1,
    step: 0.01,
    defaultValue: C.revealMaxCoverage,
    format: (v) => v.toFixed(2),
    key: 'revealMaxCoverage',
  },
];

export const ALL_CLOUD_SPECS: CloudSpec[] = [
  ...CLOUD_LAYOUT_SPECS,
  ...CLOUD_RUNTIME_SPECS,
  ...CLOUD_WISP_SPECS,
  ...CLOUD_TERRAIN_SPECS,
  ...CLOUD_LIGHTING_SPECS,
  ...CLOUD_REVEAL_SPECS,
];
