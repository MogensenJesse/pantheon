...
- **Shadows:** Sun/ambient intensity from lighting curves + day cycle (energy-gated). Ground receive uses the dense near-follow PCSS cascade (`VISUAL.shadows.lighting.near` → `sunShadow/nearCascadeShadow.ts` + `pcssShadowNode.ts` + `pcssShadowFilter.ts`). Main/far sun map is hard coverage for godrays + cloud mesh receive only. Soft cloud-cast umbras are a separate map (`cloudCastShadow.ts`). Cloud cast follow runs in `gameTick.ts` when `sun.intensity > 0`. Night uses player glow only.
...
5b. `updatePropLod` (distance-band map prop InstancedMeshes into lod0/1/2)
...
This project is indexed by GitNexus as **pantheon** (44548 symbols, 139357 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
